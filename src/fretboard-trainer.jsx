...
const STORE_KEY = "fretmap-progress-v1";
const emptyStore = () => ({
  xp: 0, totalCorrect: 0, totalAnswered: 0, bestStreakEver: 0,
  lastDay: null, dayStreak: 0,
  noteStats: {}, // note -> { ok, n }
  lastSession: null, // { correct, total, avg, xp }
});
const loadStore = () => {
  try { return { ...emptyStore(), ...(JSON.parse(localStorage.getItem(STORE_KEY)) || {}) }; }
  catch (e) { return emptyStore(); }
};
const saveStore = (st) => { try { localStorage.setItem(STORE_KEY, JSON.stringify(st)); } catch (e) {} };
const localDay = (d = new Date()) => `${d.getFullYear()}-${d.getMonth() + 1}-${d.getDate()}`;

// streak milestones (à la guitarorb): hit these in a row for bonus XP
const STREAK_BONUS = { 5: 25, 10: 50, 20: 100, 40: 200 };
const XP_CORRECT = 10;
// level curve: 200 XP for level 2, each level ~20% more
const levelFromXp = (xp) => {
  let level = 1, need = 200, rem = xp;
  while (rem >= need) { rem -= need; level++; need = Math.round(need * 1.2); }
...
export default function FretboardTrainer() {
  ...
  const [focusWeak, setFocusWeak] = useState(true); // adaptive: drill your weak spots more
  ...
  const [summary, setSummary] = useState(null); // shown after Stop: this session vs last
  ...
  const roundRef = useRef(null); // current round for callbacks that outlive the render (timeout)
  ...
  // notes you miss most (≥4 attempts, under 85% accuracy) — the adaptive drill leans on these
  const weakNotesIn = (pool) => {
    const ns = storeRef.current.noteStats;
    return pool
      .filter((n) => ns[n] && ns[n].n >= 4 && ns[n].ok / ns[n].n < 0.85)
      .sort((a, b) => ns[a].ok / ns[a].n - ns[b].ok / ns[b].n)
      .slice(0, 3);
  };
  ...
  const finish = (type, extra = {}) => {
    ...
    // XP: base per correct, speed bonus when beating half the clock, streak milestone bonuses
    let gain = 0, bonus = 0;
    if (ok) {
      gain = XP_CORRECT;
      if (pace === "timed" && elapsed < limitRef.current / 2) gain += 5;
      bonus = STREAK_BONUS[newStreak] || 0;
      gain += bonus;
    }
    ...
    setStats((st) => ({
      ...
      total: st.total + 1,
      streak: newStreak,
      best: Math.max(st.best, newStreak),
      timeSum: st.timeSum + (ok ? elapsed : 0),
    }));
    setSessionXp((x) => x + gain);

    // lifetime progress: totals, per-note accuracy, day streak
    setStore((prev) => {
      const today = localDay();
      const yesterday = localDay(new Date(Date.now() - 864e5));
      const ns = { ...prev.noteStats };
      const target = roundRef.current?.targetNote;
      if (target) {
        const cur = ns[target] || { ok: 0, n: 0 };
        ns[target] = { ok: cur.ok + (ok ? 1 : 0), n: cur.n + 1 };
      }
      return {
        ...prev,
        xp: prev.xp + gain,
        ...
        bestStreakEver: Math.max(prev.bestStreakEver, newStreak),
        noteStats: ns,
        lastDay: today,
        dayStreak: prev.lastDay === today ? prev.dayStreak : prev.lastDay === yesterday ? prev.dayStreak + 1 : 1,
      };
    });

    setFeedback({ type, bonus, bonusStreak: newStreak, ...extra });
    setPhase("feedback");
  ...
  const stop = () => {
    ...
    setFeedback(null);
    setProgress(1);
    // session recap vs your previous session
    if (stats.total > 0) {
      const cur = {
        correct: stats.correct,
        total: stats.total,
        avg: stats.correct ? stats.timeSum / stats.correct : null,
        xp: sessionXp,
      };
      setSummary({ cur, prev: storeRef.current.lastSession });
      setStore((p) => ({ ...p, lastSession: cur }));
      setStats({ correct: 0, total: 0, streak: 0, best: 0, timeSum: 0 });
      setSessionXp(0);
    }
  };
  ...
  const sendFeedback = () => {
    ...
    if (!text) return;
    const tag = fbType === "bug" ? "Bug" : fbType === "idea" ? "Idea" : "Feedback";
    const title = encodeURIComponent(`[${tag}] ${text.slice(0, 60)}${text.length > 60 ? "…" : ""}`);
    const body = encodeURIComponent(`${text}\n\n---\n_sent from the Fretmap feedback form_`);
    window.open(`https://github.com/${FEEDBACK_REPO}/issues/new?title=${title}&body=${body}`, "_blank", "noopener");
    setFbText("");
  };
  ...
  const lv = levelFromXp(store.xp);
  const lifetimeAcc = store.totalAnswered ? Math.round((store.totalCorrect / store.totalAnswered) * 100) : null;
  const weakNow = weakNotesIn(answerPool);
  const notesWithData = answerPool.filter((n) => store.noteStats[n]?.n > 0);
  ...
  return (
    <div className="ft-root">
      <style>{`
        ...
        .ft-badges{display:flex;gap:10px;align-items:center;font-size:12px}
        .ft-lvl{background:var(--brass);color:#241804;font-weight:800;border-radius:8px;padding:4px 10px}
        .ft-flame{color:var(--brass2);font-weight:600}
        ...
        .ft-bonus{position:absolute;top:8px;right:12px;background:var(--brass2);color:#241804;font-weight:800;font-size:12px;
          border-radius:8px;padding:4px 10px;animation:ftpop .3s ease-out}
        @keyframes ftpop{0%{transform:scale(.6);opacity:0}100%{transform:scale(1);opacity:1}}
        .ft-summary{text-align:center;font-size:13px;color:var(--dim);line-height:1.9}
        .ft-summary b{color:var(--brass2);font-size:16px}
        .ft-summary .up{color:var(--good)} .ft-summary .down{color:var(--bad)}
        ...
        .ft-stats .xp b{color:var(--brass2)}
        .ft-hint{color:var(--dim);font-size:11px;margin-top:8px;padding:0 6px}
        .ft-panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;margin-top:14px;padding:16px 18px}
        .ft-panel h2{font-family:'Fraunces',serif;font-size:17px;margin:0 0 4px;color:var(--ink)}
        .ft-panel .ft-sub2{color:var(--dim);font-size:12px;margin:0 0 12px}
        .ft-lvlbar{height:8px;border-radius:6px;background:var(--panel2);overflow:hidden;margin:8px 0 4px}
        .ft-lvlbar i{display:block;height:100%;background:linear-gradient(90deg,var(--brass),var(--brass2));border-radius:6px}
        .ft-lvlrow{display:flex;justify-content:space-between;font-size:11px;color:var(--dim)}
        .ft-life{display:flex;gap:22px;flex-wrap:wrap;margin-top:12px;color:var(--dim);font-size:12px}
        .ft-life b{color:var(--ink);font-size:15px;margin-right:5px}
        .ft-weak{margin-top:12px;font-size:12px;color:var(--dim)}
        .ft-chip{display:inline-block;background:var(--bad);color:#2a0f0a;font-weight:800;border-radius:7px;padding:3px 9px;margin:0 4px}
        .ft-notebars{display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:8px 16px;margin-top:12px}
        .ft-nb{display:flex;align-items:center;gap:8px;font-size:12px}
        .ft-nb b{width:22px;color:var(--ink)}
        .ft-nb .bar{flex:1;height:7px;border-radius:5px;background:var(--panel2);overflow:hidden}
        .ft-nb .bar i{display:block;height:100%;border-radius:5px}
        .ft-nb .pct{width:34px;text-align:right;color:var(--dim);font-size:11px}
        .ft-fbrow{display:flex;gap:10px;flex-wrap:wrap;align-items:flex-start}
        .ft-fbta{flex:1;min-width:240px;font-family:inherit;font-size:13px;color:var(--ink);background:var(--panel2);
          border:1px solid var(--line);border-radius:10px;padding:10px 12px;min-height:64px;resize:vertical;box-sizing:border-box}
        .ft-fbta:focus-visible{outline:2px solid var(--brass2);outline-offset:1px}
        .ft-fbsend{font-family:inherit;font-weight:800;font-size:13px;padding:11px 18px;border-radius:10px;border:0;
          cursor:pointer;background:var(--brass);color:#241804;align-self:flex-end}
        .ft-fbsend:disabled{opacity:.4;cursor:default}
        @keyframes ftpulse{0%,100%{r:13}50%{r:16}}
        .ft-pulse{animation:ftpulse 1s ease-in-out infinite}
        @media (prefers-reduced-motion: reduce){ .ft-pulse{animation:none} .ft-bonus{animation:none} }
      `}</style>
      ...
      <div className="ft-wrap">
        ...
        <div className="ft-prompt">
          {phase === "idle" && summary && (
            <div className="ft-summary">
              <div>session: <b>{summary.cur.correct}/{summary.cur.total}</b> correct
                {summary.cur.avg != null && <> · avg <b>{summary.cur.avg.toFixed(2)}s</b></>}
                {" "}· <b>+{summary.cur.xp} XP</b></div>
              {summary.prev && summary.prev.total > 0 && (() => {
                const accNow = summary.cur.correct / summary.cur.total;
                const accPrev = summary.prev.correct / summary.prev.total;
                const dAcc = Math.round((accNow - accPrev) * 100);
                const dSpd = summary.cur.avg != null && summary.prev.avg != null ? summary.prev.avg - summary.cur.avg : null;
                return (
                  <div>vs last session:{" "}
                    <span className={dAcc >= 0 ? "up" : "down"}>{dAcc >= 0 ? "▲" : "▼"} {Math.abs(dAcc)}% accuracy</span>
                    {dSpd != null && <>{" · "}
                      <span className={dSpd >= 0 ? "up" : "down"}>{dSpd >= 0 ? "▲" : "▼"} {Math.abs(dSpd).toFixed(2)}s {dSpd >= 0 ? "faster" : "slower"}</span></>}
                  </div>
                );
              })()}
              <div style={{ color: "var(--dim)", fontSize: 11 }}>hit Start to beat it</div>
            </div>
          )}
          ...
          {phase === "feedback" && feedback?.bonus > 0 && (
            <div className="ft-bonus">{feedback.bonusStreak} in a row! +{feedback.bonus} XP</div>
          )}

          {phase === "playing" && pace === "timed" && (
            <div className={`ft-timer ${progress < 0.2 ? "low" : ""}`} style={{ width: `${progress * 100}%` }} />
          )}
        </div>
        ...
        <div className="ft-controls">
          ...
          <div className="ft-field">
            <span>pace</span>
            <div className="ft-seg">
              <button className={pace === "relaxed" ? "on" : ""} onClick={() => setPace("relaxed")}>relaxed</button>
              <button className={pace === "timed" ? "on" : ""} onClick={() => setPace("timed")}>timed</button>
            </div>
          </div>
          {pace === "timed" && (
            <div className="ft-field">
              ...
              <div className="ft-range">
                <input className="ft-slider" type="range" min="2" max="15" step="0.5" value={timeLimit}
        ...
        <div className="ft-stats">
          ...
          <span className="xp"><b>+{sessionXp}</b>XP this session</span>
        </div>
        <div className="ft-hint">
          progression: relaxed → timed → specific string → find all · streaks of 5/10/20/40 earn bonus XP
        </div>

        {/* lifetime progress */}
        <div className="ft-panel">
          <h2>Your progress</h2>
          <p className="ft-sub2">saved on this device — every correct answer earns XP, streaks earn bonuses</p>
          <div className="ft-lvlbar"><i style={{ width: `${Math.min(100, (lv.into / lv.need) * 100)}%` }} /></div>
          <div className="ft-lvlrow">
            <span>Level {lv.level}</span>
            <span>{lv.into} / {lv.need} XP to level {lv.level + 1}</span>
          </div>
          <div className="ft-life">
            <span><b>{store.xp}</b>total XP</span>
            <span><b>{store.totalCorrect}/{store.totalAnswered}</b>lifetime correct{lifetimeAcc != null ? ` (${lifetimeAcc}%)` : ""}</span>
            <span><b>{store.bestStreakEver}</b>best streak ever</span>
            <span><b>{store.dayStreak || 0}</b>day streak</span>
          </div>
          {weakNow.length > 0 && (
            <div className="ft-weak">
              weak spots — the drill is serving these more often:
              {weakNow.map((n) => <span key={n} className="ft-chip">{n}</span>)}
            </div>
          )}
          {notesWithData.length > 0 && (
            <div className="ft-notebars">
              {notesWithData.map((n) => {
                const d = store.noteStats[n];
                const pct = Math.round((d.ok / d.n) * 100);
                const col = pct >= 85 ? "var(--good)" : pct >= 60 ? "var(--brass)" : "var(--bad)";
                return (
                  <div key={n} className="ft-nb">
                    <b>{n}</b>
                    <span className="bar"><i style={{ width: `${pct}%`, background: col }} /></span>
        ...
        <div className="ft-panel">
          <h2>Feedback</h2>
          <p className="ft-sub2">how are you using Fretmap? what's missing, broken, or great? — it lands straight in the project tracker</p>
          <div className="ft-fbrow">
            <div className="ft-seg" style={{ alignSelf: "flex-start" }}>
              <button className={fbType === "idea" ? "on" : ""} onClick={() => setFbType("idea")}>💡 idea</button>
              <button className={fbType === "bug" ? "on" : ""} onClick={() => setFbType("bug")}>🐞 bug</button>
              <button className={fbType === "other" ? "on" : ""} onClick={() => setFbType("other")}>💬 other</button>
            </div>
            <textarea className="ft-fbta" placeholder="e.g. I use this 10 minutes before band practice — would love a left-handed mode"
              value={fbText} onChange={(e) => setFbText(e.target.value)} />
            <button className="ft-fbsend" disabled={!fbText.trim()} onClick={sendFeedback}>Send feedback</button>
          </div>
          <div className="ft-hint" style={{ padding: 0, marginTop: 8 }}>
            opens a prefilled GitHub issue — or browse <a href={`https://github.com/${FEEDBACK_REPO}/issues`} target="_blank" rel="noopener" style={{ color: "var(--brass2)" }}>what others said</a>
          </div>
        </div>
      </div>
    </div>
...
ReactDOM.createRoot(document.getElementById("root")).render(<FretboardTrainer />);